...
initial.values_trend <- list( Mu.phiPB = rnorm(Nclass_phiPB, 0.9, 0.6),
                              ...
                              sigma.eps.phiPB = runif(1,0,3),
                              sigma.eps.psiPB = runif(1,0,3),
                              sigma.eps.rhoPB = runif(1,0,3),
                              sigma.eps.phiB = runif(Nclass_phiB,0,3),
                              sigma.eps.psiB = runif(Nclass_psiB,0,3),
                              eps.phiPB = rnorm(n_cohorts, 0, runif(1,0,3)),
                              eps.psiPB = rnorm(n_cohorts, 0, runif(1,0,3)),
                              eps.rhoPB = rnorm(n_cohorts, 0, runif(1,0,3)),
                              eps.phiB = array(rnorm(Nclass_phiB * n_cohorts, mean = 0, sd = runif(1,0,3)), dim = c(Nclass_phiB, n_cohorts)),
                              eps.psiB = array(rnorm(Nclass_psiB * n_cohorts, mean = 0, sd = runif(1,0,3)), dim = c(Nclass_psiB, n_cohorts)),
                              ...
                              phiPB = array(0, c(Nclass_phiPB, n_cohorts)),
                              psiPB = array(0, c(Nclass_psiPB, n_cohorts)), # These ones need to be initialized here as squares that do not change are filled in the model
                              rhoPB = array(0, c(Nclass_psiPB, n_cohorts)), # These ones need to be initialized here as squares that do not change are filled in the model
                              phiB = array(0, c(Nclass_phiB, n_cohorts)),
                              psiB = array(0, c(Nclass_psiB, n_cohorts))
                              )

parameters.to.save <- c("Mu.phiPB", "Mu.psiPB", "Mu.rhoPB",
                        "Mu.phiB", "Mu.psiB", "Mu.rhoB",
                        "pPB","pB","alp", 
                        "eps.phiPB", "eps.psiPB", "eps.rhoPB",
                        "eps.phiB", "eps.psiB")
...
save(output_dd, rb, waic, file = paste("outputs/", m_, "_",  sex_select, ".Rdata", sep = ""))
