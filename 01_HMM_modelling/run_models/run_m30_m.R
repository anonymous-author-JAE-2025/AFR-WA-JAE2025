...
COV.psi <- read_xlsx(paste(DataDir, "/data_cov/", "survival_cov.xlsx", sep = "")) %>% 
  select(ENSO, Season, sex) %>% 
  filter(sex == sex_select) %>% 
  filter(Season >= 1970 & Season <= 2020) %>% 
  mutate(std_ENSO = (ENSO - mean(ENSO)) / sd(ENSO)) %>% 
  pull(std_ENSO)

COV.psi <- c(COV.psi[1:36], rep(COV.psi[36], 51 - 36))
