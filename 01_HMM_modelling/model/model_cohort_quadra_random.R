model_cohort_quadra_random <- nimbleCode({
  ...
  for (c in 1:n_cohorts){
    eps.phi[c] ~ dnorm(0, sd=sdeps.phi)
    eps.psi[c] ~ dnorm(0, sd=sdeps.psi)
  }
  sdeps.phi ~ dunif(0,10)
  sdeps.psi ~ dunif(0,10)
  
  #b1phiPB ~ dnorm(0,sd=2)
  #b2phiPB ~ dnorm(0,sd=2)
  
  b1psiPB ~ dnorm(0,sd=2)
  b2psiPB ~ dnorm(0,sd=2)
  ...
  for (c in 1:n_cohorts){
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,c]) <- Mu.phiPB[ac] + eps.phi[c] #+ (b1phiPB * cohort.phi[c]) + (b2phiPB * cohort.phi[c]^2) + eps.phi[c]
    }
    #for(ac in 2:Nclass_psiPB){#age from 6 to 15
    for(ac in 1:Nclass_psiPB){#age from 6 to 15
      logit(psiPB[ac,c]) <- Mu.psiPB[ac] + (b1psiPB * cohort.psi[c]) + (b2psiPB * cohort.psi[c]^2) + eps.psi[cpsi[c]]
  ...
  for(c in 1:n_cohorts){
    for(t in (c):(n_year)){
      ...
      gamma[1,2,c,t] <- phiPB[AgeclassphiPB[(t-c+1)],c] * psiPB[AgeclasspsiPB[(t-c+1)],cpsi[c]] * rhoPB[AgeclasspsiPB[(t-c+1)]]
      gamma[1,3,c,t] <- phiPB[AgeclassphiPB[(t-c+1)],c] * psiPB[AgeclasspsiPB[(t-c+1)],cpsi[c]] * (1-rhoPB[AgeclasspsiPB[(t-c+1)]])
