m_t_cov_phi_psi <- nimbleCode({
  ...
  for(ac in 1:Nclass_phiPB){Mu.phiPB[ac]  ~ dnorm(0.9,0.6)}
  
  #temporal effects
  #for (j in 1:2){
  # bphiPB[j]  ~ dnorm(0,sd=2)
  # bcphiPB[j]  ~ dnorm(0,sd=2)
  #}
  b1cpsiPB  ~ dnorm(0,sd=2)
  b2cpsiPB  ~ dnorm(0,sd=2)
  b3cpsiPB  ~ dnorm(0,sd=2)
  
  b1cphiPB  ~ dnorm(0,sd=2)
  b2cphiPB  ~ dnorm(0,sd=2)
  b3cphiPB  ~ dnorm(0,sd=2)
  ...
  for (t in 1:n_year ){
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,t]) <- Mu.phiPB[ac] + b1cphiPB*COVphi1[t] + b2cphiPB*COVphi2[t] + b3cphiPB*COVphi3[t] 
    }
    #for(ac in 2:Nclass_psiPB){#age from 6 to 15
    for(ac in 1:Nclass_psiPB){#age from 6 to 15
      logit(psiPB[ac,t]) <- Mu.psiPB[ac] + b1cpsiPB*COVpsi1[tpsi[t]] + b2cpsiPB*COVpsi2[tpsi[t]] + b3cpsiPB*COVpsi3[tpsi[t]] 
      #logit(psiPB[ac,t]) <- Mu.psiPB[ac-1] + bpsiPB*YEARpsi[t]+ 
    }
  } 
  #for(ac in 2:Nclass_psiPB){logit(rhoPB[ac]) <- Mu.rhoPB[ac-1]}
  for(ac in 1:Nclass_psiPB){logit(rhoPB[ac]) <- Mu.rhoPB[ac]}
  for (s in 1:Nclass_psiB){
  ...
  for(c in 1:n_cohorts){
    for(t in (c):(n_year)){
      gamma[1,1,c,t] <- phiPB[AgeclassphiPB[(t-c+1)],t] * (1 - psiPB[AgeclasspsiPB[(t-c+1)],tpsi[t]])
      gamma[1,2,c,t] <- phiPB[AgeclassphiPB[(t-c+1)],t] * psiPB[AgeclasspsiPB[(t-c+1)],tpsi[t]] * rhoPB[AgeclasspsiPB[(t-c+1)]]
      gamma[1,3,c,t] <- phiPB[AgeclassphiPB[(t-c+1)],t] * psiPB[AgeclasspsiPB[(t-c+1)],tpsi[t]] * (1-rhoPB[AgeclasspsiPB[(t-c+1)]])
