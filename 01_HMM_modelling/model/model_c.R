model_c <- nimbleCode({
  ...
  for(c in 1:n_cohorts) {
    eps.phi[c] ~ dnorm(0, sd = sigma.eps.phi)
    eps.psi[c] ~ dnorm(0, sd = sigma.eps.psi)
    eps.rho[c] ~ dnorm(0, sd = sigma.eps.rho)
  }
  ...
  for (c in 1:n_cohorts){
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,c]) <- Mu.phiPB[ac] + b.phi.PB * COV.phi[c] + eps.phi[c]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(psiPB[ac,c]) <- Mu.psiPB[ac] + b.psi.PB * COV.psi[c] + eps.psi[c]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(rhoPB[ac,c]) <- Mu.rhoPB[ac] + b.rho.PB * COV.rho[c] + eps.rho[c]
    }
  } 
