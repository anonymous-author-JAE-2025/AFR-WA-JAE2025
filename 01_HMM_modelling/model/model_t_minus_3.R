model_t_minus_3 <- nimbleCode({
  ...
  for (t in 4:n_year){  # Start from 4 to use t-3 for covariates and random effects
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,t]) <- Mu.phiPB[ac] + b.phi.PB * COV.phi[t-3] + eps.phi[t-3]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(psiPB[ac,t]) <- Mu.psiPB[ac] + b.psi.PB * COV.psi[t-3] + eps.psi[t-3]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(rhoPB[ac,t]) <- Mu.rhoPB[ac] + b.rho.PB * COV.rho[t-3] + eps.rho[t-3]
    }
  }
  
  # Handle the first three years separately (t=1, t=2, and t=3)
  for (t in 1:3) {
    for(ac in 1:Nclass_phiPB){
