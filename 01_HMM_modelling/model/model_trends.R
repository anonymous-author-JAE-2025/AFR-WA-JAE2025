model_trends <- nimbleCode({
  ...
  for (c in 1:n_cohorts) {
    for (ac in 1:Nclass_phiPB) {
      logit(phiPB[ac,c]) <- Mu.phiPB[ac] + b1phiPB * cohort.phi[c] + b2phiPB * cohort.phi[c]^2
    }
    
    for (ac in 1:Nclass_psiPB){
      logit(psiPB[ac,c]) <- Mu.psiPB[ac] + b1psiPB * cohort.psi[c] + b2psiPB * cohort.psi[c]^2
    }
  } 
  
  for(ac in 1:Nclass_psiPB) {
    logit(rhoPB[ac]) <- Mu.rhoPB[ac]
  }
