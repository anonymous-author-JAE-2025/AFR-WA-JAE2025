model_t_minus_2 <- nimbleCode({
  ...
  for (t in 3:n_year){  # Start from 3 to use t-2 for covariates and random effects
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,t]) <- Mu.phiPB[ac] + b.phi.PB * COV.phi[t-2] + eps.phi[t-2]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(psiPB[ac,t]) <- Mu.psiPB[ac] + b.psi.PB * COV.psi[t-2] + eps.psi[t-2]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(rhoPB[ac,t]) <- Mu.rhoPB[ac] + b.rho.PB * COV.rho[t-2] + eps.rho[t-2]
    }
  }
  
  # Handle the first two years separately (t=1 and t=2)
  for (t in 1:2) {
    for(ac in 1:Nclass_phiPB){
