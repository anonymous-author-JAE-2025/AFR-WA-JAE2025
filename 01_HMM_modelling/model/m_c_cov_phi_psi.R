m_c_cov_phi_psi <- nimbleCode({
  ...
  for (c in 1:n_cohorts){
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,c]) <- Mu.phiPB[ac] + b1cphiPB*COVphi1[c] + b2cphiPB*COVphi2[c] + b3cphiPB*COVphi3[c]
    }
    #for(ac in 2:Nclass_psiPB){#age from 6 to 15
    for(ac in 1:Nclass_psiPB){#age from 6 to 15
      logit(psiPB[ac,c]) <- Mu.psiPB[ac] + b1cpsiPB*COVpsi1[c] + b2cpsiPB*COVpsi2[c] + b3cphiPB*COVpsi3[c]
