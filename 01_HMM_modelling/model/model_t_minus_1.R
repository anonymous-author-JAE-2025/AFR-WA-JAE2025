model_t_minus_1 <- nimbleCode({
  ...
  for (t in 2:n_year){  # Start from 2 to use t-1 for covariates and random effects
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,t]) <- Mu.phiPB[ac] + b.phi.PB * COV.phi[t-1] + eps.phi[t-1]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(psiPB[ac,t]) <- Mu.psiPB[ac] + b.psi.PB * COV.psi[t-1] + eps.psi[t-1]
    }
    for(ac in 1:Nclass_psiPB) {
      logit(rhoPB[ac,t]) <- Mu.rhoPB[ac] + b.rho.PB * COV.rho[t-1] + eps.rho[t-1]
    }
  }
  
  # Handle the first year separately (t=1)
  for(ac in 1:Nclass_phiPB){
    logit(phiPB[ac,1]) <- Mu.phiPB[ac]
  }
  for(ac in 1:Nclass_psiPB) {
    logit(psiPB[ac,1]) <- Mu.psiPB[ac]
  }
  for(ac in 1:Nclass_psiPB) {
    logit(rhoPB[ac,1]) <- Mu.rhoPB[ac]
  }
