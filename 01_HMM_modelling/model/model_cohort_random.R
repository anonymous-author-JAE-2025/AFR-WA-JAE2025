model_cohort_random <- nimbleCode({
  ...
  for (c in 1:n_cohorts){
    for(ac in 1:Nclass_phiPB){
      logit(phiPB[ac,c]) <- Mu.phiPB[ac] + bphiPB * cohort.phi[c] + eps.phi[c]
    }
    #for(ac in 2:Nclass_psiPB){#age from 6 to 15
    for(ac in 1:Nclass_psiPB){#age from 6 to 15
      logit(psiPB[ac,c]) <- Mu.psiPB[ac] + bpsiPB * cohort.psi[c] + eps.psi[cpsi[c]]
